        ...
        switch (choixMenu) {
            ...
            case '4': {
                ...
                if (listeLexiques == NULL || listeLexiques->suivant == NULL) {
                    printf("(!!) Il faut au moins deux lexiques pour une fusion.\n");
                    break;
                }

                printf("Sélectionnez le lexique A (celui qui recevra les mots) :\n");
                t_lexique* lexiqueA = choisirLexique(listeLexiques);
                if (lexiqueA == NULL) {
                    printf("(!!) Aucun lexique A sélectionné.\n");
                    break;
                }

                printf("Sélectionnez le lexique B (celui à fusionner dans A) :\n");
                t_lexique* lexiqueB = choisirLexique(listeLexiques);
                if (lexiqueB == NULL || lexiqueB == lexiqueA) {
                    printf("(!!) Sélection invalide. Les deux lexiques doivent être différents.\n");
                    break;
                }
                ...
                printf("(!) La fusion de '%s' <- '%s' a été réalisée avec succès.\n", lexiqueA->nom, lexiqueB->nom);
                break;
            }
            ...
            case '5': {
                ...
                printf("(!) Le fichier a été importé dans le lexique '%s'.\n", lexiqueChoisi->nom);
                break;
            }
